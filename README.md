...
This project demonstrate the core idea of using terrafrom and ansible in a pipeline workflow. Every push to the main branch triggers the pipeline to test the developer's code, in our scenatio it is an index.html file. The terraform is triggered then confirms with its portal about the state file statues, it decide after that to deploy or destroy. The authentication with the cloud is with OpenID Connect for short-lived credentials assumed by terrafrom. After provisioning, Ansible role comes and the public ip passed by the pipeline to generate dynamic inventory file and then ansible configure the ec2 by downloading nginx then copy the index file to the default path of nginx. After that a validation happend that everything works correctly. Last job is a TTL count for 2 hours or any time you predeict your test will finish, after this ttl a destroy job started. This demonstarte that every tool has its own role and job. Terrafrom is resonsible for provison infrastructure, on the hand Ansible is about infrastrucre configuration management while the pipeline is the deamon that drive all this workflow in automated way.
